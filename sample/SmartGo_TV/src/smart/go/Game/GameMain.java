...
import smart.go.MainApp;
import smart.go.Animation.Animation;
import smart.go.Animation.AnimationGroupSeq;
import smart.go.Animation.EasingCurve;
...
import smart.go.Element.ElementBound;
import smart.go.Element.ElementList;
import smart.go.GameCommand.GameCommand;
import smart.go.Sound.GameSound;
...
{	
	...
	public ArrayList<Integer> m_have_cardme = new ArrayList<Integer>();
	public ArrayList<Integer> m_have_cardyou = new ArrayList<Integer>();
	public ArrayList<Integer> m_eat_cardme = new ArrayList<Integer>();
	public ArrayList<Integer> m_eat_cardyou = new ArrayList<Integer>();
	public ArrayList<Integer> m_bottom_card = new ArrayList<Integer>();
	public ArrayList<Integer> m_card_deck = new ArrayList<Integer>();

	public ArrayList<Integer> m_sortMyCard = new ArrayList<Integer>();
	public ArrayList<Integer> m_remote = new ArrayList<Integer>();
	...
	public void KARAMset() {	// nID만 저장
		for(int i=0; i<10; i++) {
			if(true == m_player[player_me].existPlayCard(i)) {
				Card pMyCard = getCard(m_player[player_me].getPlayCard(i));
				m_have_cardme.add(pMyCard.getID());
			}	// 내가 가진패
			if(true == m_player[player_you].existPlayCard(i)) {
				Card pYouCard = getCard(m_player[player_you].getPlayCard(i));
				m_have_cardyou.add(pYouCard.getID());
			}	// 상대방이 가진패
		}

		int size = m_player[player_me].m_total.size();
		for(int i=0; i<size; i++) {
			m_eat_cardme.add(m_player[player_me].m_total.get(i));
		}		// 내가 먹은패
		size = m_player[player_you].m_total.size();
		for(int i=0; i<size; i++) {
			m_eat_cardyou.add(m_player[player_you].m_total.get(i));
		}		// 상대방이 먹은패

		m_player[player_me].m_total.clear();
		m_player[player_you].m_total.clear();

		for(int i=0; i<m_gameCard.size(); i++) {
			Card CardDeck = m_gameCard.get(i);
			m_card_deck.add(CardDeck.getID());
		}		// 카드덱

		for(int i=0; i<10; i++) {
			for(int j=0; j<4; j++) {
				if(m_boardCard[i][j].bExist == true)
					m_bottom_card.add(m_boardCard[i][j].nID);
			}
		}
		cli.setHaveMeList(m_have_cardme);
		cli.setHaveYouList(m_have_cardyou);
		cli.setEatMeList(m_eat_cardme);
		cli.setEatYouList(m_eat_cardyou);
		cli.setBottomList(m_bottom_card);
		cli.setCardDeckList(m_card_deck);

		m_player[player_me].clearPlayCard();
		m_player[player_you].clearPlayCard();
	...
	public void onEndAni(int nID)
	{
		...
		if( (nID >= Animation.id_send_first_card_to_you) && (nID <= Animation.id_send_first_card_to_you + 5) )			// 처음줄 5장
		{
			int n = nID - Animation.id_send_first_card_to_you;

			getCard( m_player[player_you].getPlayCard(n) ).setVisible( false );
		...
		if( (nID >= Animation.id_send_second_card_to_you) && (nID <= Animation.id_send_second_card_to_you + 5) )
		{
			int n = nID - Animation.id_send_second_card_to_you + 5;
		...
		if( nID == Animation.id_send_youcard_to_board )
		{
			showCardEffect();
			sendBackCardToBoard();
			return;
		...
		if( nID == Animation.id_dummy_you_turn )
		{
			comAct();
			return;
		}
		...
		if( nID >= Animation.id_eat_card_kwang && nID <= Animation.id_eat_card_pi )
		{
			...
			{
				m_player[player_you].onEndAi( nID );
			}			
			return;
		}
	...
	{
		m_player[player_you] = new GamePlayer();
		m_player[player_you].initMe( this );
		m_player[player_you].setPosTotalPoint( 162, 8 );
		m_player[player_you].setPosKwang( 215, 60 );
		m_player[player_you].setPosMung( 320, 60 );
		m_player[player_you].setPosDDi( 450, 60 );
		m_player[player_you].setPosPi( 320, 0 );
	...
	void registTurnMark()
	{
		m_turnPos[player_you].nX = 7;
		m_turnPos[player_you].nY = 0;
	...
	void registSunMark()
	{
		m_sunPos[player_you].nX = 100;
		m_sunPos[player_you].nY = 40;
	...
	public void sendFirstCardToCom()
	{
		for( int i=0; i<5; i++ )
		{
			...
			m_player[player_you].setPlayCard( i, findCard.getID() );		

			m_eliList.setTopOrder( findCard.getID() );

			findCard.setMoveAni( Animation.id_send_first_card_to_you + i, EasingCurve.Linear,
	...
	public void sendSecondCardToCom()
	{
		for( int i=0; i<5; i++ )
		{
			int nIndex = (int) m_gameCard.size() - 1;
			...
			m_player[player_you].setPlayCard( i+5, findCard.getID() );		

			m_eliList.setTopOrder( findCard.getID() );		

			findCard.setMoveAni( Animation.id_send_second_card_to_you + i, EasingCurve.Linear,
	...
	public void sendSecondCardToMe()
	{
		for( int i=0; i<5; i++ )
		{
			int nIndex = (int) m_gameCard.size() - 1;
	...
	public void preKARAM() {
		Log.e("preKARAM", "preKARAM");
	...
	public void KARAMget() 
	{
		m_player[player_me].initVariable();
		m_player[player_you].initVariable();

		Log.e("KARAMget", "KARAMget");
		m_have_cardme = cli.getHaveMeList();
		Log.e("HM", ""+m_have_cardme.size());

		m_have_cardyou = cli.getHaveYouList();
		Log.e("HY", ""+m_have_cardyou.size());

		m_eat_cardme = cli.getEatMeList();
		Log.e("EM", ""+m_eat_cardme.size());

		m_eat_cardyou = cli.getEatYouList();
		Log.e("EY", ""+m_eat_cardyou.size());

		m_bottom_card = cli.getBottomList();
		Log.e("BB", ""+m_bottom_card.size());

		m_card_deck = cli.getCardDeckList();		
		Log.e("BC", ""+m_card_deck.size());

		for(int i=0; i<5; i++) {
			Card findCard = getCard(m_have_cardme.get(i));
			...
			findCard.setMoveAni( Animation.id_send_first_card_to_me + i, EasingCurve.Linear,
					findCard.getPosX(), findCard.getPosY(), m_ptMyCard[i].nX, m_ptMyCard[i].nY, 0.5f );
			getCard(m_player[player_me].getPlayCard(i)).setFront();
		}
		if(m_have_cardme.size() > 5 ) {
			for(int i=0; i<(m_have_cardme.size()-5); i++) {
				Card findCard = getCard(m_have_cardme.get(i+5));
				...
				getCard(m_player[player_me].getPlayCard(i+5)).setFront();
			}
		}
		Log.e("HM", "HM for문 완료");

		for(int i=0; i<5; i++) {
			Card findCard = getCard(m_have_cardyou.get(i));
			...
			getCard(m_player[player_you].getPlayCard(i)).setVisible(false);
			m_miniCard[i].setVisible(true);
		}
		if(m_have_cardyou.size() > 5 ) {
			for(int i=0; i<(m_have_cardyou.size()-5); i++) {
				Card findCard = getCard(m_have_cardyou.get(i+5));
				...
				getCard(m_player[player_you].getPlayCard(i+5)).setVisible(false);
				m_miniCard[i].setVisible(true);
			}
		}
		Log.e("HY", "HY for문 완료");
		...
		for(int i=0; i<4; i++) {
			Card findCard = getCard(m_bottom_card.get(i));
		...
		if(m_bottom_card.size() > 4) {
			for(int i=0; i<(m_bottom_card.size()-4); i++) {
				Card findCard = getCard(m_bottom_card.get(i+4));
		...
		Log.e("BB", "BB for문 완료");
		m_nBoardCardCount = m_bottom_card.size();

		m_gameCard.clear();
		for(int i=0; i<m_card_deck.size(); i++) {
			Card CardDeck = getCard(m_card_deck.get(i));
			m_gameCard.add(CardDeck);
		}		// 카드덱
		Log.e("BC", "BC for문 완료");


		for(int i=0; i<m_eat_cardme.size(); i++) {
			Card findCard = getCard(m_eat_cardme.get(i));
			m_player[player_me].addCard( findCard );
			findCard.setFront();
		}
		Log.e("EM", "EM for문 완료");

		for(int i=0; i<m_eat_cardyou.size(); i++) {
			Card findCard = getCard(m_eat_cardyou.get(i));
			m_player[player_you].addCard( findCard );
			Log.e("EY", ""+findCard.getID());
			findCard.setFront();
			m_player[player_you].onAniDDi();
			m_player[player_you].onAniKwang();
			m_player[player_you].onAniMung();
			m_player[player_you].onAniPi();
		}	
		m_player[player_you].sortDDi();
		m_player[player_you].sortKwang();
		m_player[player_you].sortMung();
		m_player[player_you].sortPi();
		Log.e("EY", "EY for문 완료");
	...
	public void KARAMremote() {
		...
		for(int i=0; i<10; i++) {
			...
			Card pMyCard = getCard(m_player[player_me].getPlayCard(i));
			m_remote.add(pMyCard.getID());

		}
		cli.setRemote(m_remote);	
		c.WakeUp();
		c.Wait();
		myTurn();
	}
	...
	public void sendFirstCardToBoard()
	{
		for( int i=0; i<4; i++ )
		{
			int nIndex = (int) m_gameCard.size() - 1;
	...
	public void sendSecondCardToBoard()
	{
		for( int i=0; i<4; i++ )
		{
			int nIndex = (int) m_gameCard.size() - 1;
	...
	public void youTurn()
	{
		showTurnMark( player_you );

		/*List<Integer> eat_list;
		eat_list = new ArrayList<Integer>();		
		eat_list = cli.getEatList();
		 */
		endTurn();

		dummyAnimation( Animation.id_dummy_you_turn, 0.5f);
	}
	...
	public void comAct()
	{
		...
		for( int i=0; i<GamePlayer.MAX_PLAY_CARD; i++ )
		{
			if( m_player[player_you].existPlayCard(i) == false )
				continue;

			Card pCard = getCard(m_player[player_you].getPlayCard(i) );		
	...
	public void sendCardAni( Card pCard, int nAniID )
	{
		float fScale = 2.0f;
		if( true == m_bMyTurn )
		{
	...
	public void sendComCardToBoard( int nIndex )
	{
		...
		// 낼 카드
		Card pComCard = getCard( m_player[player_you].getPlayCard(nIndex) );
		...
		{			
			sendCardAni( pComCard, Animation.id_send_youcard_to_board );
			...
			m_player[player_you].removePlayCard( nIndex );	
	...
	public void sendMyCardToBoard( int nIndex )
	{
		setTouchMyCard( false );
	...
	public void game_start()
	{
		...
		if( m_whoSun == player_me )
		{
			...
			m_cmd.registCommand( GameCommand.id_send_first_card_to_you, 0.5f );
			...
			m_cmd.registCommand( GameCommand.id_send_second_card_to_you, 2.0f );	
		...
		else
		{
			...
			m_cmd.registCommand( GameCommand.id_send_first_card_to_you, 1.0f );		

			m_cmd.registCommand( GameCommand.id_send_second_card_to_board, 1.5f );
			m_cmd.registCommand( GameCommand.id_send_second_card_to_me, 2.0f );
			m_cmd.registCommand( GameCommand.id_send_second_card_to_you, 2.5f );
		}
	...
	public void shuffleCard()
	{
		...
		cardDeck = cli.getList();
		m_whoSun = cli.getOrder();

		for( int i=0; i<Card.MAX_CARD; i++ )				// for(i=0; i<48; i++) 
		{
			Card tmpCard = m_card[cardDeck.get(i)];  	// Card[48] m_card  값이 400부터 447		=> 1월부터 12월까지 차례대로 저장
			m_gameCard.add( tmpCard );
		}
		for( int i=0; i<max_board_card; i++ )			// for(i=0; i<10; i++)
		{
			for( int j=0; j<4; j++ )
				clearBoardCard( i, j );
		}
	...
	public void showGo()
	{
		...
		{
			m_player[player_you].updateGo();
			nGo = m_player[player_you].getGo();
		}
	...
	{	
		...
		if( nEatCount > 0 )
		{
			for( int i=0; i<nEatCount; i++ )
			{
				...
				//cli.setEatList(m_curEatCard.get(i));
				if( true == m_bMyTurn )
					m_player[player_me].addCard( pCard );
				else
					m_player[player_you].addCard( pCard );
	...
	public void sortMyCard() {
		for(int i=0; i<10; i++) {
			...
			Card pMyCard = getCard(m_player[player_me].getPlayCard(i));
			m_sortMyCard.add(pMyCard.getID());
		}	// 내가 가진패
	}

	public void sortMyCardset() {
		for(int i=0; i<5; i++) {
			if(m_sortMyCard.size() > i){
				Card findCard = getCard(m_sortMyCard.get(i));
			...
			{
				m_player[player_me].setPlayCard(i, -1);
			}
		}
		if(m_sortMyCard.size() > 5 ) {
			for(int i=0; i<5; i++) {
				if(m_sortMyCard.size() > (i + 5)){
					Card findCard = getCard(m_sortMyCard.get(i+5));
				...
				{
					m_player[player_me].setPlayCard(i+5, -1);
				}
			}
		}
		else{
	...
	public void sortYouCard() {
		for(int i=0; i<GamePlayer.MAX_PLAY_CARD; i++) {
			if( false == m_player[player_you].existPlayCard(i) )
				continue;

			Card pMyCard = getCard(m_player[player_you].getPlayCard(i));
			m_sortMyCard.add(pMyCard.getID());
		}	// 내가 가진패
	}

	public void sortYouCardset() {
		for(int i=0; i<GamePlayer.MAX_PLAY_CARD; i++){
			...
			if(m_sortMyCard.size() > i){
				...
				m_player[player_you].setPlayCard(i, -1);
		}
		
		for(int i=0; i<GamePlayer.MAX_PLAY_CARD; i++){
			m_miniCard[i].setVisible(false);
		}
		for(int i=0; i<m_sortMyCard.size(); i++){
			m_miniCard[i].setVisible(true);
		}
		m_sortMyCard.clear();
	}
		...
		if( m_nAddCount > 0 )
		{
			...
			for( int i=0; i<m_nAddCount; i++ )
			{
				if( true == m_player[otherMan].havePi() )
				{
					...
					if( true == m_bMyTurn )
						m_player[player_me].addCard( m_player[player_you].getPi() );
					else
						m_player[player_you].addCard( m_player[player_me].getPi() );
	...
	public void newTurn()
	{
		...
		{
			if( true == m_player[player_you].canGo() )
			{
				comSelectGoStop();
			}
			else
	...
	public void myTurn()
	{
		...
		if(remote_mode == 0)
			...
			int nFindIndex;
			nFindIndex = cli.getmIndex();
			sendMyCardToBoard(nFindIndex);
		}
