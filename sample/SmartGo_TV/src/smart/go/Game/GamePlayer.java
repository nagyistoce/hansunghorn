...
import smart.go.Element.VNumber;
...
public class GamePlayer
{
	...
	public Vector<Integer> m_total = new Vector<Integer>();
	...
	private int[]	m_playCard = new int[MAX_PLAY_CARD];// 가진패
	...
	public void deletePi()
	{
		int num;
		int nLast = (int) m_pi.size() - 1;
		num = m_pi.get(nLast).getID();
		m_pi.remove( nLast );
		
		for(int i=0; i<m_total.size(); i++){
			if(num == m_total.get(i))
				m_total.remove(i);
		}
	...
	/*public void nscreenSort() {
		int nSize = m_pMain.m_eat_cardyou
		for( int i=0; i<m_pMain.; i++ )
		{
			m_pi.get(i).setPos( m_ptPi.nX + i * GAP, m_ptPi.nY );
			m_pMain.setTopOrder( m_pi.get(i).getID() );
		}
