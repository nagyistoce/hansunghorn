...
-(void)send:(SODServerInfo *)serverInfo withMessag:(char *)message
{
    ...
    if(sendto(serverInfo.sockfd, message, 1024, 0, (struct sockaddr*)&server, server_len) == -1){
...
-(NSString *)receive:(SODServerInfo *)serverInfo
{
    ...
    if(recvfrom(serverInfo.sockfd, (void *)msg, 1024, 0, (struct sockaddr*)&server, &server_len) == -1){
        NSLog(@"fail to receive message\n");
    }
