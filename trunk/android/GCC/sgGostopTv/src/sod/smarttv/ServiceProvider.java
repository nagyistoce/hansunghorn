...
public class ServiceProvider {
	...
	//	Packet packet = new packet
		...
		if( isText(openServiceFile.getName())){ // 4.서비스파일의 타입을 넣는다 (int)
			//4. 텍스트라면
			servicePacket.push(ServiceProvider.TEXT_TYPE);
		}
		else{//4. 이미지라면
			servicePacket.push(ServiceProvider.IMAGE_TYPE);
			
			String absoluteFilePath = openServiceFile.getAbsoluteFilePath();
			Bitmap img= BitmapFactory.decodeFile(absoluteFilePath);
			
			servicePacket.push(img.getWidth() );//4.1 가로길이 넣고
			servicePacket.push(img.getHeight() );//4.2 세로길이 넣고
		}
